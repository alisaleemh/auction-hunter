...
function updateReindexStatus(payload) {
  ...
  if (indexing) {
    reindexStatus.textContent = `${payload?.index_stale ? "Stalled" : "Reindexing"}${scope ? ` (${scope})` : ""}...`;
...
function updateProgress(payload) {
  ...
  const indexing = payload?.indexing ?? initialState.indexing;
  const stale = payload?.index_stale ?? false;
  ...
  const label = message || (stale ? "Index stalled" : indexing ? "Indexing..." : "Idle");
  const indeterminate = indexing && !message && total <= 0;

  progressShell.hidden = !indexing && !stale && percent <= 0 && !message;
